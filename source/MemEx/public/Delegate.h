...
namespace SA {
	...
	class delegate<RET(PARAMS...)> final : private delegate_base<RET(PARAMS...)> {
		using MyStubType = typename delegate_base<RET(PARAMS...)>::stub_type;

	public:

		delegate() = default;

		bool isNull() const noexcept { return invocation.stub == nullptr; }
		...
		delegate& operator =(const LAMBDA& instance) {
			assign((void*)(&instance), (MyStubType)lambda_stub<LAMBDA>);
			return *this;
		} //operator =
		...
		inline void Release() noexcept {
			this->invocation.object = nullptr;
			this->invocation.stub = nullptr;
		}
